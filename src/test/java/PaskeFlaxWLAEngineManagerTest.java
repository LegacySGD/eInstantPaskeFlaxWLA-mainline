...
public class PaskeFlaxWLAEngineManagerTest
{
	...
	{
		logger = Logger.getLogger(PaskeFlaxWLAEngineManagerTest.class);
		gameParams = XMLUtil.parseResource("paymodel/PaskeFlaxWLAIW/GameParams.xml").getDocumentElement();
		previousOutcome = XMLUtil.parseResource("paymodel/PaskeFlaxWLAIW/InitOutcome.xml").getDocumentElement();
	...
	//@Ignore
	@Test
	public void test_buy_2x_10000CreditsWin_RevealThreeTimes()
		throws Exception
	{
		Element playerInput;
		int pricePoint = 10000;
		...
		TestGDSProxy.setTestGdsData(getTestGdsData("test-scenario-data"));
		playerInput = new PlayerInputBuilder().appendActionInput("play").build();
		TestODEProxy.setJsonResponse("{'prizeValue':50000000,'scenario':1}");
		outcome = OutcomeUtil.getSignedData(outcome, "Outcome");
		outcome = manager.play(InputUtil.buildInput(gameParams, XMLUtil.getElement(outcome), playerInput));
		logger.debug(outcome);
		...
		assertEquals("test-scenario-data", response.get("scenario").getAsString());
		assertEquals("Wager", JXPathUtil.getString(outcome, "//NextStage/text()"));
		assertSettledPendingPayout(outcome, pricePoint, 0, 50000000);
	}
